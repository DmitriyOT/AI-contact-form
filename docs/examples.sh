...
echo '=== 3. Валидное обращение -> 201 (ai: true/false в зависимости от AI) ==='
...
echo '=== 4. Пустое тело {} -> 422 с details по всем полям ==='
curl -s -w '\nHTTP %{http_code}\n' -X POST "$BASE/api/contact" \
  -H 'Content-Type: application/json' \
  -d '{}'

echo '=== 5. Невалидный email + короткий комментарий -> 422 ==='
curl -s -w '\nHTTP %{http_code}\n' -X POST "$BASE/api/contact" \
  -H 'Content-Type: application/json' \
  -d '{"name":"Ivan","phone":"+79001234567","email":"not-an-email","comment":"short"}'

echo '=== 6. Битый JSON -> 400 ==='
curl -s -w '\nHTTP %{http_code}\n' -X POST "$BASE/api/contact" \
  -H 'Content-Type: application/json' \
  -d '{invalid'

echo '=== 7. Неверный Content-Type -> 415 ==='
curl -s -w '\nHTTP %{http_code}\n' -X POST "$BASE/api/contact" \
  -H 'Content-Type: text/plain' \
  -d 'name=Ivan'

echo '=== 8. Rate limit: 6 запросов подряд при лимите 5/час -> последние 429 с Retry-After ==='
