...
describe('submitContact', () => {
  ...
  it('возвращает ok с message, ai и analysis при 201', async () => {
    const analysis = {
      sentiment: 'positive',
      category: 'Вопрос',
      priority: 'средний',
  ...
  it('подставляет дефолты, если в ответе 201 нет полей', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(mockResponse(201, { status: 'accepted' })),
    );
    ...
    expect(result).toEqual({
      ok: true,
      message: 'Обращение принято. Спасибо!',
      ai: false,
      analysis: null,
    });
  });
  ...
  it('429 с Retry-After: 0 даёт 0, а не дефолтные 60 секунд', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        mockResponse(
  ...
  it('подставляет дефолтное сообщение, если в теле ошибки нет message', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          mockResponse(500, { error: { code: 'internal_error' } }),
        ),
    );
